...
<script>
    function file_delete(self) {
        ...
        if (r==true) {
            var index = $(self).parents('.file-div').index(),
                id = $(self).parents('tr').attr('id');
            $.post('/admin/' + action + '/delete_file', {'id':id, 'index':index}, function(res) {
    ...
    $(document).ready(function () {
        ...
        $grid = $("#table_list_2");
        var config = [
            {'str': '名称', 'name': 'name', 'align':'center', width: 180,},
            {'str': '网址', 'name': 'url', 'align':'center', width: 300,},
